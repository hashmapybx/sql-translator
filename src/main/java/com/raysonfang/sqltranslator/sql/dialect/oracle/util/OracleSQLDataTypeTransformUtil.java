...
public class OracleSQLDataTypeTransformUtil extends SQLTransformUtils {
    ...
    public static SQLDataType transformOracleToMySql(SQLDataType x) {
        ...
        if (nameHash == FnvHash.Constants.UROWID) {
            ...
            if (arguments.size() == 0) {
                ...
                if (arguments.size() > 1) {
                    scale = ((SQLIntegerExpr) arguments.get(1)).getNumber().intValue();
                }
            ...
            int precision = 0;
            if (arguments.size() > 0) {
                precision = ((SQLIntegerExpr) arguments.get(0)).getNumber().intValue();
            }
     ...
     * @return
     */
    public static SQLDataType transformOracleToSR(SQLDataType x) {
        final String name = x.getName();
        final long nameHash = x.nameHashCode64();
        if (name == null) {
            return x;
        ...
        if (nameHash == FnvHash.Constants.UROWID) {
            ...
            /* }*/
                ...
                || nameHash == FnvHash.Constants.LONG_RAW) {
            arguments.clear();
            dataType = new SQLDataTypeImpl("VARBINARY"); //
        } else if (nameHash == FnvHash.Constants.CLOB
                || nameHash == FnvHash.Constants.NCLOB
                || nameHash == FnvHash.Constants.LONG
                || nameHash == FnvHash.Constants.XMLTYPE) {
            arguments.clear();
            dataType = new SQLCharacterDataType("VARBINARY");
            //SR中不支持BLOB CLOB 统一转为VARBINARY 不指定长度 默认的最大值为1048576 字节=1MB
        } else {
            dataType = x;
        }
